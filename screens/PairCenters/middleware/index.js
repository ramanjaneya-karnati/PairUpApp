...
import addLocaleToUserInterests from './add-locate-to-user';
import getUsersByLocale from './get-users-by-locale';
import getUserDetails from './get-user-details';
import reportUser from './report-user';
import likedUsersList from './liked-users-list';
import singleUserLikedProfileView from './single-user-liked-profile-view';

export default store => next => action => {
  next(action);
  switch (action.type) {
    case Actions.PAIR_CENTERS_COMPONENT_INIT:
      initialView(store.dispatch);
      getPairCentersData(store.dispatch);
      break;
    case Actions.ADD_USER_TO_LOCALE_INTERST:
      addLocaleToUserInterests(store, action.payload);
      break;
    case Actions.GET_USERS_BY_LOCALE:
      getUsersByLocale(store, action.payload);
      break;
    case Actions.GET_USER_DETAILS:
      getUserDetails(store, action.payload);
      break;
    case Actions.REPORT_USER_SUBMIT:
      reportUser(store, action.payload);
      break;
    case Actions.LIKED_USERS_LIST:
      likedUsersList(store, action.payload);
      break;
    case Actions.SINGLE_USER_LIKED_PROFILE_VIEW:
      singleUserLikedProfileView(store, action.payload);
      break;
  }
}
...
function initialView(dispatch) {
  ...
  dispatch(change(FORM_NAMES.PAIR_CENTERS_FORM, `${PAIR_CENTERS_FORM_KEYS.IS_SELECTED_PAIR_CENTER}`, null));
}
