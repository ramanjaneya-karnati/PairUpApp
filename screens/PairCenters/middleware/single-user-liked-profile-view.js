...
export default async function singleUserLikedProfileView(store) {
  try {
/*    const mainFormData = getFormValues(FORM_NAMES.PAIR_CENTERS_FORM)(store.getState());
    const {
      userData
    } = mainFormData;*/
    ...
    if (likedUsersList.status && likedUsersList.status.toLowerCase() === 'success') {
      ...
      store.dispatch(change(FORM_NAMES.PAIR_CENTERS_FORM, `${PAIR_CENTERS_FORM_KEYS.SINGLE_USER_LIKED_PROFILE_DATA}`, ApiData[0]));
      store.dispatch(change(FORM_NAMES.PAIR_CENTERS_FORM, `${PAIR_CENTERS_FORM_KEYS.CURRENT_VIEW}`, VIEWS.SINGLE_USER_LIKED_PROFILE_VIEW));
